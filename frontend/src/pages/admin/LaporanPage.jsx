...
const LaporanPage = () => {
  ...
  const chartDataStatusPaket = dashboardData ? [
    { name: 'Open', value: dashboardData.total_paket_open },
    { name: 'Close', value: dashboardData.total_paket_close }
  ] : [];
  ...
  const renderActiveTable = () => {
    ...
    if (currentReport.loading) {
      return <div className="p-8 text-center text-slate-500">Memuat data laporan...</div>;
    }
    
    if (!currentReport.data || currentReport.data.length === 0) {
      return <div className="p-8 text-center text-slate-500">Tidak ada data untuk laporan ini.</div>;
    }
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map(p => (
              <tr key={p.penawaran_id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4">{p.paketPengadaan?.nama_paket}</td>
    ...
    if (activeTab === 'pemenang') {
      const filtered = currentReport.data.filter(w => w.paketPengadaan?.nama_paket?.toLowerCase().includes(search.toLowerCase()));
      return (
        <table className="w-full text-left text-sm text-slate-600">
          <thead className="bg-slate-50 text-slate-700 text-xs uppercase font-semibold border-b border-slate-200">
            <tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map(w => (
              <tr key={w.pemenang_id} className="hover:bg-slate-50 transition-colors">
                <td className="px-6 py-4 font-medium text-slate-800">{w.paketPengadaan?.nama_paket}</td>
        ...
        <div className="border-b border-slate-200 bg-slate-50/80 px-6 pt-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => alert('Fitur Export PDF belum tersedia.')} className="flex items-center px-4 py-2 text-sm font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 rounded-xl transition-colors border border-rose-200">
                <Download size={16} className="mr-2" /> PDF
              </button>
              <button onClick={() => alert('Fitur Export Excel belum tersedia.')} className="flex items-center px-4 py-2 text-sm font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-colors border border-emerald-200">
